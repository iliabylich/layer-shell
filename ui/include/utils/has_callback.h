...
#define HAS_CALLBACK_TYPED_SELF(Self, name, Type)                              \
  static void __free__##name(void *) {}                                        \
                                                                               \
  static void set_##name(Self *self, Type value) {                             \
    g_object_set_data_full(G_OBJECT(self), stringify(name),                    \
                           (void *)(size_t)value, __free__##name);             \
  }                                                                            \
                                                                               \
  static Type get_##name(Self *self) {                                         \
    return (Type)(size_t)g_object_get_data(G_OBJECT(self), stringify(name));   \
  }

#define WIDGET_HAS_CALLBACK(name, Type)                                        \
  HAS_CALLBACK_TYPED_SELF(GtkWidget, name, Type)
