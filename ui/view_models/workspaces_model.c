...
static void workspaces_model_update(WorkspacesModel *self,
...
static void workspaces_model_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec) {
  WorkspacesModel *self = WORKSPACES_MODEL(object);
  switch (property_id) {
  case PROP_DATA: {
    struct IO_FFIArray_HyprlandWorkspace *data = g_value_get_pointer(value);
    if (data)
      workspaces_model_update(self, *data);
    break;
  }
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID(object, property_id, pspec);
...
static void workspaces_model_class_init(WorkspacesModelClass *klass) {
  ...
  object_class->set_property = workspaces_model_set_property;
  object_class->finalize = workspaces_model_finalize;

  properties[PROP_ITEMS] =
      g_param_spec_object("items", NULL, NULL, G_TYPE_LIST_MODEL,
                          G_PARAM_READABLE);
