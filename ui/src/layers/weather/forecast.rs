use crate::{
    ...
    widgets::weather::{DailyImages, DailyLabels, HourlyImages, HourlyLabels},
};
...
pub(crate) fn init() {
    subscribe(|event| {
        if let Event::WeatherForecast { daily, hourly } = event {
            for ((label, image), weather) in HourlyLabels().iter().zip(HourlyImages()).zip(hourly) {
            ...
            for ((label, image), weather) in DailyLabels().iter().zip(DailyImages()).zip(daily) {
                let text = format!(
                    "{}: {:>5.1}℃ - {:>5.1}℃",
