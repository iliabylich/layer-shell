...
static void sound_ready_changed(GObject *, GParamSpec *, gpointer data) {
  ...
  g_object_get(self->window_model, "ready", &ready, NULL);
...
static void sound_window_set_property(GObject *object, guint property_id,
  ...
  case PROP_WINDOW_MODEL:
    ...
    g_object_bind_property_full(
        self->window_model, "volume", self->sound_adjustment, "value",
        G_BINDING_SYNC_CREATE, transform_sound_volume, NULL, NULL, NULL);
    sound_ready_changed(NULL, NULL, self);
    g_signal_connect_object(self->window_model, "notify::ready",
                            G_CALLBACK(sound_ready_changed), self, 0);
    g_signal_connect_object(self->window_model, "notify::volume",
                            G_CALLBACK(sound_value_changed), self, 0);
    g_signal_connect_object(self->window_model, "notify::muted",
                            G_CALLBACK(sound_value_changed), self, 0);
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID(object, property_id, pspec);
...
GtkWidget *sound_window_new(GtkApplication *app,
                            SoundWindowModel *window_model) {
  return g_object_new(sound_window_get_type(), "application", app,
                      "window-model", window_model, NULL);
}
