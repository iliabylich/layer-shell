...
impl ValueEncoder {
    ...
    pub(crate) fn encode_short_str(buf: &mut EncodingBuffer, s: StringRef) {
    ...
    pub(crate) fn encode_object_path(buf: &mut EncodingBuffer, path: StringRef) {
    ...
    pub(crate) fn encode_value(buf: &mut EncodingBuffer, value: &Value) {
        match value {
            ...
            Value::StringRef(s) => Self::encode_short_str(buf, s.clone()),
            Value::LongString(s) => Self::encode_long_str(buf, s),
            Value::ObjectPath(path) => Self::encode_object_path(buf, path.clone()),
...
mod tests {
    ...
    fn test_encode_string() {
        let mut buf = EncodingBuffer::new();
        buf.encode_u8(0);
        ValueEncoder::encode_short_str(&mut buf, StringRef::new("abcd"));
    ...
    fn test_encode_object_path() {
        let mut buf = EncodingBuffer::new();
        buf.encode_u8(0);
        ValueEncoder::encode_object_path(&mut buf, StringRef::new("abcd"));
        assert_eq!(buf.done(), b"\0\0\0\0\x04\x00\x00\x00abcd\0")
    }
