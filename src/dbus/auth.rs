...
impl Auth {
    ...
    fn schedule_write_zero(&self, ring: &mut IoUring) -> Result<()> {
    ...
    fn schedule_write_auth_external(&self, ring: &mut IoUring) -> Result<()> {
        let mut sqe = ring.get_sqe()?;
    ...
    fn schedule_read_data(&mut self, ring: &mut IoUring) -> Result<()> {
        let mut sqe = ring.get_sqe()?;
    ...
    fn schedule_write_data(&self, ring: &mut IoUring) -> Result<()> {
        let mut sqe = ring.get_sqe()?;
    ...
    fn schedule_read_guid(&mut self, ring: &mut IoUring) -> Result<()> {
        let mut sqe = ring.get_sqe()?;
        sqe.prep_read(self.fd, self.buf.as_mut_ptr(), self.buf.len());
    ...
    fn schedule_write_begin(&mut self, ring: &mut IoUring) -> Result<()> {
        let mut sqe = ring.get_sqe()?;
    ...
    pub(crate) fn init(&mut self, ring: &mut IoUring) -> Result<()> {
        self.schedule_write_zero(ring)
    }

    pub(crate) fn process(&mut self, op: u8, res: i32, ring: &mut IoUring) -> Result<bool> {
        match Op::try_from(op)? {
            Op::WriteZero => {
                ensure!(res > 0);
                let written = res as usize;
                ensure!(written == 1);
                self.schedule_write_auth_external(ring)?;
                Ok(false)
            }
            ...
            Op::WriteData => {
                ...
                self.schedule_read_guid(ring)?;
                Ok(false)
            }
            Op::ReadGUID => {
                ensure!(res > 0);
                self.schedule_write_begin(ring)?;
                Ok(false)
            }
            Op::WriteBegin => {
                ensure!(res > 0);
